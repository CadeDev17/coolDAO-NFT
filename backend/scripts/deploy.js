...
const { METADATA_URL } = require("../constants.js");
...
async function main() {
  ...
  const coolDAOContract = await ethers.getContractFactory("coolNFT")

  const deployedCoolDAOContract = await coolDAOContract.deploy(metadataURL)

  await deployedCoolDAOContract.deployed()

  console.log("coolDAO NFT Contract: ", deployedCoolDAOContract.address)
}
